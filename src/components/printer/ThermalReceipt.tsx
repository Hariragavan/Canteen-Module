...
export const ThermalReceipt: React.FC<ThermalReceiptProps> = ({
  ...
  const getTamilMealName = (meal: string) => {
    ...
    if (lower.includes('tiffin') || lower.includes('breakfast')) return 'காலை டிபன்';
    if (lower.includes('lunch')) return 'மதிய உணவு';
    if (lower.includes('tea') || lower.includes('snack')) return 'தேநீர் & ஸ்நாக்ஸ்';
    return meal;
  };

  return (
    <div className="flex flex-col items-center gap-4 w-full">
      {/* 80mm/58mm ESC/POS Ultra-Compact Thermal Slip Container */}
      <div
        id="tvsSlipPrintArea"
        className="flex flex-col items-center gap-3"
      >
        {ordersList.map((ord, idx) => {
          const rateVal = ord.rate ?? 40;
          const formattedDate = getFormattedDate(ord);
          const formattedTime = getFormattedTime(ord);
          const tamilMealName = getTamilMealName(ord.meal);
          const isMultiple = ordersList.length > 1;

          return (
            <div
              key={ord.id || ord.orderUuid || idx}
              className="tvs-slip-page w-[270px] max-w-[275px] bg-white text-black font-sans p-2.5 rounded shadow-sm border border-slate-300 select-none"
            >
              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="w-[118px] shrink-0 flex flex-col items-start text-left">
                  <div className="flex items-center justify-between w-full mb-1">
                    ...
                    {isMultiple && (
                      <span className="text-[9px] font-mono font-bold bg-black text-white px-1 py-0.2 rounded">
                        {idx + 1}/{ordersList.length}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-start pl-1 text-left">
                  ...
                  <div className="border-2 border-black text-center py-0.5 px-1 font-bold text-xs sm:text-sm text-black mt-1 mb-1.5">
                    QTY: 1 {isMultiple && `(Slip ${idx + 1} of ${ordersList.length})`}
                  </div>
      ...
      <div className="flex items-center space-x-1.5 text-xs text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200 print:hidden font-medium">
        ...
        <span>
          {ordersList.length > 1 
            ? `${ordersList.length} Individual Token Slips Generated (Different QRs)` 
            : 'Compact Thermal Slip Dispensed & Ready'}
        </span>
      </div>
    </div>
  );
